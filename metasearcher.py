...
corpus_index = ['sensor_number','actuator_number','constant0','equal','gaurd','lambdagraph','recurse','identity','constant1','constant2','constant3']
...
def extend_a_terminalnode(search_graph,extendable_node,type_compatible_node_links,gen,corpus_of_objects):
...
def eval_graph(search_graph,terminalnode,PHASE):
	...
	try:
		...
		terminalnode.executed = 1
	except world_exception: #### Invalid world action
		terminalnode.world_failed = 1
	except time_exception:
		pass
	except:
		terminalnode.semantic_failed = 1
	finally:
...
def extend_graph(search_graph,corpus_index,corpus_of_objects,init_type_compatible_node_links):
	gen = [extendable_nodes for extendable_nodes in copy.copy(search_graph.terminalnodes) if  extendable_nodes.semantic_failed == 0 and extendable_nodes.world_failed == 0] #extendable_nodes.executed == 1 and
	type_compatible_node_links = init_type_compatible_node_links
	for extendable_node in gen: ### for all executed terminal nodes
		search_graph = extend_a_terminalnode(search_graph,extendable_node,type_compatible_node_links,gen,corpus_of_objects)
...
search_graph = metasearcher(corpus_index,init_world,corpus_of_objects,init_type_compatible_node_links)				
